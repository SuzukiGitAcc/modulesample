...
public class DialerActivity extends Activity implements LinphoneGuiListener {
	...
	private String getStatusIcon(RegistrationState state) {
		if (state == RegistrationState.RegistrationOk)
			return Integer.toString(R.get("drawable", "status_green"));
		
		else if (state == RegistrationState.RegistrationNone)
			return Integer.toString(R.get("drawable", "status_red"));
		
		else if (state == RegistrationState.RegistrationProgress)
			return Integer.toString(R.get("drawable", "status_orange"));
		
		else 
			return Integer.toString(R.get("drawable", "status_offline"));
	}
	
	private void displayRegisterStatus() {
		ListView accounts = (ListView) findViewById(R.get("id", "accounts"));
		if (accounts != null) {
			...
			Adapter adapterForList = new SimpleAdapter(this, hashMapAccountsStateList, R.get("layout", "accounts"),
	                new String[] {"Identity", "State"},
	                new int[] { R.get("id", "Identity"), R.get("id", "State") });
	...
	private void verifiyAccountsActivated() {
		...
		int nbAccounts = prefs.getInt(getString(R.get("string", "pref_extra_accounts")), 0);
		...
		for (int i = 0; i < nbAccounts; i++) {
			...
			if (createdByWizard && !activated) {
				...
				String username = prefs.getString(getString(R.get("string", "pref_username_key")) + key, "");
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.get("layout", "dialer"));

		mAddress = (AddressText) findViewById(R.get("id", "SipUri")); 
		EraseButton erase = (EraseButton) findViewById(R.get("id", "Erase"));
		erase.setAddressWidget(mAddress);
		erase.requestFocus();

		mCall = (CallButton) findViewById(R.get("id", "Call"));
		mCall.setAddressWidget(mAddress);
		mBack = (Button) findViewById(R.get("id", "Back"));
		mAddCall = (Button) findViewById(R.get("id", "AddCall"));
		mInCallControls = (LinearLayout) findViewById(R.get("id", "InCallControls"));
		mStatus =  (TextView) findViewById(R.get("id", "status_label"));
			...
			mAddCall.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					try {
						...
						Log.e(e);
						Toast.makeText(DialerActivity.this, R.get("string", "error_adding_new_call"), Toast.LENGTH_LONG).show();
					}
				}
			});
		}
		...
		SlidingDrawer drawer = (SlidingDrawer) findViewById(R.get("id", "drawer"));
		if (drawer != null) {
			boolean disable_sliding_drawer = getResources().getBoolean(R.get("bool", "disable_dialer_sliding_drawer"));
		...
		AddressAware numpad = (AddressAware) findViewById(R.get("id", "Dialer"));
		if (numpad != null)
    ...
    private synchronized void tryToInitTabletUI() {
    	...
    	mVideoCaptureView = (CameraView) findViewById(R.get("id", "video_background"));
		...
		mSwitchCamera = (Button) findViewById(R.get("id", "switch_camera"));
	...
	public void onAlreadyInCall() {
		showToast(R.get("string", "warning_already_incall"));
	}

	@Override
	public void onCannotGetCallParameters() { 
		showToast(R.get("string", "error_cannot_get_call_parameters"),mAddress.getText());
	}

	@Override
	public void onWrongDestinationAddress() {
		showToast(R.get("string", "warning_wrong_destination_address"), mAddress.getText());
	}
