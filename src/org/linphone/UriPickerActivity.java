...
public class UriPickerActivity extends TabActivity implements ContactPicked {
	...
	public static class DialerActivity extends Activity implements OnClickListener {
		...
		protected void onCreate(Bundle savedInstanceState) {
			setContentView(R.get("layout", "simplified_dialer"));
			mAddress = (AddressText) findViewById(R.get("id", "SipUri"));

			addButton = (Button) findViewById(R.get("id", "AddCallButton"));
//			addButton.setCompoundDrawablePadding(100);
			...
			if (EXTRA_PICKER_TYPE_ADD.equals(type)) {
				addButton.setCompoundDrawablesWithIntrinsicBounds(0, R.get("drawable", "incall_add_small"), 0, 0);
				addButton.setText(getString(R.get("string", "AddCallButtonText")));
			} else if (EXTRA_PICKER_TYPE_TRANSFER.equals(type)) {
				addButton.setCompoundDrawablesWithIntrinsicBounds(0, R.get("drawable", "picker_transfer"), 0, 0);
				addButton.setText(getString(R.get("string", "TransferCallButtonText")));
			} else {
				throw new RuntimeException("unknown type");
			}

//			findViewById(Common.getR("id", "AddCallCancelButton")).setOnClickListener(this);

			((AddressAware)findViewById(R.get("id", "Dialer"))).setAddressWidget(mAddress);
			((AddressAware)findViewById(R.get("id", "Erase"))).setAddressWidget(mAddress);
			super.onCreate(savedInstanceState);
		}
