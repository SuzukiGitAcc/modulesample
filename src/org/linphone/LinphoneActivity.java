...
public class LinphoneActivity extends TabActivity implements ContactPicked
	...
	public void onCreate(Bundle savedInstanceState) {
		activity = TiApplication.getInstance().getAppCurrentActivity();
...
//		mAlwaysChangingPhoneAngle = rotation;
		...
		useFirstLoginActivity = getResources().getBoolean(R.get("bool", "useFirstLoginActivity"));
		useMenuSettings = getResources().getBoolean(R.get("bool", "useMenuSettings"));
		useMenuAbout = getResources().getBoolean(R.get("bool", "useMenuAbout"));
		checkAccount = !useFirstLoginActivity;

		SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(this);

		if (!useFirstLoginActivity || pref.getBoolean(getString(R.get("string", "first_launch_suceeded_once_key")), false)) {
			fillTabHost();
		} else {
...
//				onFirstLaunch();
					...
					&& !checkDefined(pref, R.get("string", "pref_username_key"), R.get("string", "pref_domain_key"))) {
//				onBadSettings(pref);
			} else {
				checkAccount = false;
			}
...
//				Toast.makeText(this, getString(Common.getR("string", "ec_calibration_launch_message")), Toast.LENGTH_LONG).show();
//				try {
...
//							.edit().putBoolean(
//									getString(Common.getR("string", "pref_echo_canceller_calibration_key")),
...
//				}
	...
	private synchronized void fillTabHost() {
	    ...
	    startActivityInTab("history",
	    		...
	    		R.get("string", "tab_history"), R.get("drawable", "history_orange"));

	    
	    startActivityInTab(DIALER_TAB,
	    		new Intent().setClass(this, DialerActivity.class).setData(getIntent().getData()),
	    		R.get("string", "tab_dialer"), R.get("drawable", "dialer_orange"));
	    ...
	    startActivityInTab("contact",
	    		...
	    		R.get("string", "tab_contact"), R.get("drawable", "contact_orange"));
	...
	protected void onNewIntent(Intent intent) {
		...
		if (DialerActivity.instance() != null) {
			...
			Toast.makeText(this, getString(R.get("string", "dialer_null_on_new_intent")), Toast.LENGTH_LONG).show();
		}
	}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		inflater.inflate(R.get("menu", "linphone_activity_menu"), menu);
		menu.findItem(R.get("id", "menu_settings")).setVisible(useMenuSettings);
		menu.findItem(R.get("id", "menu_about")).setVisible(useMenuAbout);
		return true;
	}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.get("id", "menu_settings")).setVisible(!lc.isIncall());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.get("id", "menu_settings")) {
			startprefActivity();
			return true;
		}
		else if (id == R.get("id", "menu_exit")) {
			finish();
			stopService(new Intent(ACTION_MAIN)
				.setClass(this, LinphoneService.class));
		}
		else if (id == R.get("id", "menu_about")) {
	...
	void showPreferenceErrorDialog(String message) {
		if (!useMenuSettings) {
			...
			.setMessage(String.format(getString(R.get("string", "config_error")), message))
			.setCancelable(false)
			.setPositiveButton(getString(R.get("string", "yes")), new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					startprefActivity();
				}
			})
			.setNegativeButton(getString(R.get("string", "no")), new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int id) {
					dialog.cancel();
				}
			});
...
//
//		lDialogTextView.setText(Html.fromHtml(getString(R.get("string", "first_launch_message"))));
//
//		builder.setCustomTitle(lDialogTextView)
//		.setCancelable(false)
//		.setPositiveButton(getString(R.get("string", "cont")), new DialogInterface.OnClickListener() {
//			public void onClick(DialogInterface dialog, int id) {
...
//		lDialogTextView.setPadding(10, 10, 10, 10);
//
//		lDialogTextView.setText(Html.fromHtml(getString(R.get("string", "initial_config_error"))));
//
//		builder.setCustomTitle(lDialogTextView)
//		.setCancelable(false)
//		.setPositiveButton(getString(R.get("string", "yes")), new DialogInterface.OnClickListener() {
...
//				checkAccount = false;
//			}
//		}).setNeutralButton(getString(R.get("string", "no")), new DialogInterface.OnClickListener() {
//			public void onClick(DialogInterface dialog, int id) {
//				dialog.cancel();
//				checkAccount = false;
//			}
//		}).setNegativeButton(getString(R.get("string", "never_remind")), new DialogInterface.OnClickListener() {
//			public void onClick(DialogInterface dialog, int id) {
