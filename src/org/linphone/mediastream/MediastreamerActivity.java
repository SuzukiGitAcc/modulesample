...
public class MediastreamerActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final SurfaceView previewSurface = (SurfaceView) findViewById(R.get("id", "video_capture_surface"));
		final SurfaceHolder holder = previewSurface.getHolder();
		holder.setType(SurfaceHolder.SURFACE_TYPE_PUSH_BUFFERS);

		/* retrieve rendering surface */
		GLSurfaceView view = (GLSurfaceView) findViewById(R.get("id", "video_surface"));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		inflater.inflate(R.get("menu", "videocall_activity_menu"), menu);
		
		if (Build.VERSION.SDK_INT < 9)
			menu.findItem(R.get("id", "videocall_menu_change_camera")).setVisible(false);
		else {
		...
		if (videoCodec.equals(VP8_MIME_TYPE))
			menu.findItem(R.get("id", "videocall_menu_codec_vp8")).setChecked(true);
		else if (videoCodec.equals(MPEG4_MIME_TYPE))
			menu.findItem(R.get("id", "videocall_menu_codec_mpeg4")).setChecked(true);
		
		switch (bitrate) {
			case 64:   
				menu.findItem(R.get("id", "videocall_menu_bitrate_64_kbps")).setChecked(true); break;
			case 128: 
				menu.findItem(R.get("id", "videocall_menu_bitrate_128_kbps")).setChecked(true); break;
			case 256:  
				menu.findItem(R.get("id", "videocall_menu_bitrate_256_kbps")).setChecked(true); break;
			case 512:  
				menu.findItem(R.get("id", "videocall_menu_bitrate_512_kbps")).setChecked(true); break;
			case 1024: 
				menu.findItem(R.get("id", "videocall_menu_bitrate_1024_kbps")).setChecked(true); break;
		}
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if(id == R.get("id", "videocall_menu_exit"))
		{
			this.finish();
		}
		else if(id == R.get("id", "videocall_menu_change_camera"))
		{
			cameraId = (cameraId + 1) % Camera.getNumberOfCameras();
			changeCamera(cameraId, nativeObj);
			setVideoPreviewWindowId(findViewById(R.get("id", "video_capture_surface")), nativeObj);
		}
		else if(id == R.get("id", "videocall_menu_codec_vp8"))
		{
			updateVideoCodec(VP8_MIME_TYPE);
		}
		else if(id == R.get("id", "videocall_menu_codec_mpeg4"))
		{
			updateVideoCodec(MPEG4_MIME_TYPE);
		}
		else if(id == R.get("id", "videocall_menu_bitrate_64_kbps"))
		{
			updateBitrate(64);
		}
		else if(id == R.get("id", "videocall_menu_bitrate_128_kbps"))
		{
			updateBitrate(128);
		}
		else if(id == R.get("id", "videocall_menu_bitrate_256_kbps"))
		{
			updateBitrate(256);
		}
		else if(id == R.get("id", "videocall_menu_bitrate_512_kbps"))
		{
			updateBitrate(512);
		}
		else if(id == R.get("id", "videocall_menu_bitrate_1024_kbps"))
		{
			updateBitrate(1024);
		}
